...
void MainWindow::on_openButton_clicked()
{
    ...
    if(ui->openButton->text()==QString("打开串口"))
    {
        ...
        timer=new QTimer;
        connect(timer,&QTimer::timeout,this,&MainWindow::updateGet);
        timer->start(1);//每隔1毫秒读取一次
        //read之前必须调用readyRead()
        //connect(serial,&QSerialPort::readyRead,this,&MainWindow::updateGet);
    }
    //如果在串口打开状态下点击button
...
{
      //connect(serial,&QSerialPort::bytesWritten,this,&MainWindow::updateSend);
    timer2=new QTimer;
    timer2->setSingleShot(true);
    connect(timer2,&QTimer::timeout,this,&MainWindow::updateSend);
    timer2->start(1);//一毫秒后发送，然后停止发送
}
